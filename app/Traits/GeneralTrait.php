...
trait GeneralTrait
{
    ...
    public function saveFile($file, $path, $group_id){
        $fileExtension = $file->getClientOriginalExtension();
        $fileName = $file->getClientOriginalName();

        $fileNameSeparated = preg_split ("/\./", $fileName); 

        $isExist = File::select('name', 'id')->where('name', 'like', $fileNameSeparated[0].'%')
                        ->where('group_id', '=', $group_id)
                        ->get();

        if($isExist->isEmpty()){

            //this is the first file uploaded to group
            $path = $path;
        ...
        else if(!$isExist->isEmpty()){
            //insert the number of row inside it
            
            $newFileName = '';
            $lastRecord = $isExist[count($isExist)-1]['name'];
            preg_match_all('!\d+!', $lastRecord, $matches);
            $filtered = array_filter($matches);
            if (!empty($filtered)) {
                $newNumber = intval($matches[0][0])+1;
                $newFileName = $fileNameSeparated[0].$newNumber.'.'.$fileNameSeparated[1];
           
            }
           
            else{
                $newFileName = $fileNameSeparated[0].'1.'.$fileNameSeparated[1];
            }
